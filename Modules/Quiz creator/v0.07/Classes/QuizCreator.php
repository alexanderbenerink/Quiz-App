<?php
  ...
  class QuizCreator {
    ...
    private function drawQuestionType( $questionNumber ) {
    ...
    public function drawConnectFieldset( $questionNumber, $inputAmount ) {
      ...
      for ( $i = 0; $i < $inputAmount; $i++ ) {
        ...
        $fieldset .= "
          <fieldset>
            <legend>Set $number</legend>
            <input type='text' name='connect-$questionNumber-$number' required>
            <input type='text' name='connect_answ-$questionNumber-$number' required>
          </fieldset>
        ";
      }
      $fieldset .= "
    ...
    private  function drawQuestionPoints( $questionPoints, $questionNumber ) {
      return "
        <fieldset>
    ...
    private function drawQuestionTime( $questionTime, $questionNumber ) {
      return "
        <fieldset>
          <legend>Time</legend>
    ...
    private function assignSelected( $type, $returnValue ) {
      ...
      for ( $i = 0; $i < $numbOfQuestions; $i++ ) {
        ...
        switch ( $_POST["type-q$number"] ) {
          ...
          case "images";
            break;
          case "connect";
            $this -> answers[$i] = $this -> getConnectAnswers( $number );
            break;
        }
      }
      /* Vanaf hier zijn alle resultaten verzameld
        ...
        $answers is een associative array met betrekking tot de quiz vraag antwoorden en bevat de volgende waardes:
          ...
          * Antwoorden van true/ false vragen
            - True of False
          * Antwoorden van connect vragen
            - connect-1, answer-1, connect-2, answer-2, connect-3, answer-3

        Dingen om te overwegen wanneer de tijd aanwezig is:
          * Elke quiz krijgt een code ter referentie
          * Validatie van input velden
            - Geen null waarde waar illegaal
            - Geen duplicate quiz names in database wanneer de quiz geen referentie code heeft
      */
    }

    // Upload quiz naar database
    public function uploadQuiz() {
      // Code voor datebase upload hier
      // Wordt via index.php al aangeroepen
      print_r($this -> quiz);
    ...
    public function displayResults() {
      ...
      for ( $i = 0; $i < count($this -> questions); $i++ ) {
        ...
        switch ( $type ) {
          case "multiple-choice";
            ...
            $incorrect_1 = $this -> answers[$i]["incorrect-1"];
            $incorrect_2 = $this -> answers[$i]["incorrect-2"];
            $incorrect_3 = $this -> answers[$i]["incorrect-3"];
            $content = "
              Correct answer: $correct <br>
              Incorrect answer 1: $incorrect_1 <br>
              Incorrect answer 2: $incorrect_2 <br>
              Incorrect answer 3: $incorrect_3 <br>
            ";
            $results .= $content;
            break;
          case "true/false";
            ...
            break;
          case "connect";
            $connect_1 = $this -> answers[$i]["connect-1"];
            $answer_1 = $this -> answers[$i]["answer-1"];
            $connect_2 = $this -> answers[$i]["connect-2"];
            $answer_2 = $this -> answers[$i]["answer-2"];
            $connect_3 = $this -> answers[$i]["connect-3"];
            $answer_3 = $this -> answers[$i]["answer-3"];
            $content = "
              Connect 1: $connect_1 <br>
              Answer 1: $answer_1 <br>
              Connect 2: $connect_2 <br>
              Answer 2: $answer_2 <br>
              Connect 3: $connect_3 <br>
              Answer 3: $answer_3 <br>
            ";
            $results .= $content;
            break;
        }
      ...
      $results .= "
          <form action='home.php'>
            <input type='submit' value='Terug naar home'>
          </form>
        </div>
      ";
      return ($results);
    ...
    private function getQuizAttributes( $numbOfQuestions ) {
    ...
    private function getQuestionAttributes( $number ) {
    ...
    private function getMultipleChoiceAnswers( $number ) {
    ...
    private function getTrueFalseAnswer( $number ) {
      return array( "answer" => $_POST["true-false_select-$number"] );
    }

    // Geeft een array terug met de antwoorden van een connect vraag
    private function getConnectAnswers( $number ) {
      return array( "connect-1" => $_POST["connect-$number-1"], "answer-1" => $_POST["connect_answ-$number-1"], "connect-2" => $_POST["connect-$number-2"], "answer-2" => $_POST["connect_answ-$number-2"], "connect-3" => $_POST["connect-$number-3"],  "answer-3" => $_POST["connect_answ-$number-3"] );
    }
